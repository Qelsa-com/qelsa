  ...
  handler: async (ctx, args) => {
    ...
    if (existingExperiences.length === 0) {
      for (const [index, row] of profile.experiences.entries()) {
        ...
        const company_id = await findOrCreateNamed(ctx, "companies", row.company);
        const job_title_id = await findOrCreateNamed(ctx, "job_titles", row.title);
        const start_date = parseFlexibleDate(row.start) ?? Date.now();
        const end_date = row.is_current ? undefined : parseFlexibleDate(row.end);
        ...
        if (row.description) {
          await ctx.db.insert("responsibilities", {
            ...
            title: row.description.slice(0, 280),
          });
        }
      }
    }
    ...
    if (existingEducations.length === 0) {
      for (const [index, row] of profile.educations.entries()) {
        ...
        const start_year = row.start_year ?? row.end_year ?? new Date().getUTCFullYear();
...
async function findOrCreateNamed<T extends NamedTable>(
  ...
  const hits = await ctx.db
    ...
    .take(5);
  const match = hits.find((row) => row.name.toLowerCase() === trimmed.toLowerCase());
  if (match) return match._id as Id<T>;
...
async function findDegreeId(ctx: MutationCtx, name: string) {
  ...
  const hits = await ctx.db
    ...
    .take(5);
  const match =
    hits.find((row) => row.name.toLowerCase() === trimmed.toLowerCase()) ??
    hits.find((row) => row.abbreviation?.toLowerCase() === trimmed.toLowerCase()) ??
    hits[0];
  return match?._id;
}
...
async function findCityId(ctx: MutationCtx, location: string) {
  ...
  const hits = await ctx.db
    ...
    .take(5);
  const match = hits.find((row) => row.name.toLowerCase() === cityName.toLowerCase()) ?? hits[0];
  return match?._id;
}
...
function parseFlexibleDate(value?: string) {
  ...
  const year = Number.parseInt(value.replace(/[^0-9]/g, "").slice(0, 4), 10);
  if (year >= 1970 && year <= 2100) return Date.UTC(year, 0, 1);
  return undefined;
}
